...
sub send_json_tags {
    ...
    if (   ( defined( url_param('callback') ) )
        && ( length( url_param('callback') ) < 100 ) )
    {
        ...
        $json_prefix =
          'if(typeof(Insipid) == \'undefined\') Insipid = {}; Insipid.tags = {';
        $json_suffix = '}';
    }
...
sub send_json_posts {
        ...
        && ( length( url_param('callback') ) < 100 ) )
    {
        ...
        $json_prefix =
'if(typeof(Insipid) == \'undefined\') Insipid = {}; Insipid.posts = [';
        $json_suffix = ']';
    }
...
"$sql where ($tbl_bookmarks.title || $tbl_bookmarks.description like ?)";
    ...
    while ( @hr = $sth->fetchrow_array ) {
        ...
        if ( $hr[0] ne $last{id} ) {
            ...
            json_show_post( 0, $last{id}, $last{title}, $last{description},
    ...
    if ( $last{id} ne -1 ) {
        json_show_post( 1, $last{id}, $last{title}, $last{description},
            $last{access_level}, $last{url}, $last{tags}, $last{timestamp} );
    }
...
sub json_show_post {
    ...
    if ( $access_level eq 0 ) {
        $json_txt =
          $json_txt . '"u":"' . $site_url . '/insipid.cgi?go=' . $id . '",';
